class Argument:
    ...
    async def getAdmin(self, message):
        return (await message._client.get_chat_member(message.chat.id, message.from_user.id)).status in (self.pyrogram.enums.ChatMemberStatus.ADMINISTRATOR, self.pyrogram.enums.ChatMemberStatus.OWNER)
