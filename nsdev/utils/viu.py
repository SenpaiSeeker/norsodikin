...
class ViuDownloader:
    def __init__(self, cookies_file_path: str = "cookies.txt", download_path: str = "downloads"):
        ...
        if not os.path.exists(self.cookies_file_path):
            raise FileNotFoundError(f"File cookie Viu tidak ditemukan di: {self.cookies_file_path}")
    ...
    def _sync_get_info_with_ytdlp(self, url: str) -> SimpleNamespace:
        ydl_opts = {
            'quiet': True,
        ...
        with YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                title = info.get('title', 'viu_video')
                m3u8_url = info.get('url')

                if not m3u8_url:
                    raise ValueError(f"yt-dlp tidak dapat mengekstrak URL M3U8. Cookie mungkin tidak valid atau konten memerlukan premium.\n\n{info}")
    ...
    async def _get_m3u8_url(self, video_url: str) -> SimpleNamespace:
        self.log.print(f"{self.log.CYAN}Mengekstrak detail video dengan yt-dlp...")
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, self._sync_get_info_with_ytdlp, video_url)
    ...
    async def download(self, url: str) -> str:
        video_details = await self._get_m3u8_url(url)
