...
class ImageManipulator(FontManager):
        ...
        with Image.open(io.BytesIO(image_bytes)).convert("RGBA") as img:
            ...
            if bottom_text:
                ...
                draw_text_with_outline(
                    wrapped_bottom, ((img.width - bottom_w) / 2, img.height - bottom_h - 10), "center"
                )
    ...
    def _create_quote_sync(self, text: str, user_name: str, pfp_bytes: bytes = None, invert: bool = False) -> bytes:
        ...
        draw.text(
            (img_width - name_width - padding, img_height - 40), f"— {user_name}", font=font_name, fill=name_color
        )
    ...
    async def create_profile_card(
        ...
        return await self._run_in_executor(
            self._create_profile_card_sync, pfp_bytes, name, username, user_id, bio, pfp_count, is_sudo
        )
