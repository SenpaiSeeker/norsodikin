...
class PinterestAPI:
    ...
    async def _request(
        ...
        try:
            response = await client.request(
                method=method, url=url, headers=self._get_headers(), params=params, json=json_data
            )
            return await self._handle_response(response)
        except httpx.RequestError as e:
            raise PinterestAPIError(f"Request failed: {str(e)}")

    async def exchange_code_for_token(self, code: str, redirect_uri: str, client_id: str, client_secret: str) -> Dict:
    ...
    async def refresh_access_token(
        self, refresh_token: str, client_id: str, client_secret: str, continuous_refresh: bool = True
    ) -> Dict:
        data = {
    ...
    async def list_boards(
        self, page_size: Optional[int] = None, bookmark: Optional[str] = None, privacy: Optional[str] = None
    ) -> Dict:
        params = {}
        if page_size:
    ...
    async def create_board(self, name: str, description: Optional[str] = None, privacy: str = "PUBLIC") -> Dict:
    ...
    async def list_pins(self, page_size: Optional[int] = None, bookmark: Optional[str] = None) -> Dict:
        params = {}
        if page_size:
            params["page_size"] = page_size
        if bookmark:
    ...
    async def create_pin(
        ...
        data = {"board_id": board_id, "media": media}
        if title:
            data["title"] = title
        if description:
            data["description"] = description
    ...
    async def list_board_pins(
        self, board_id: str, page_size: Optional[int] = None, bookmark: Optional[str] = None
    ) -> Dict:
        params = {}
        if page_size:
            params["page_size"] = page_size
    ...
    async def search_partner_pins(
        self, query: str, page_size: Optional[int] = None, bookmark: Optional[str] = None
    ) -> Dict:
        params = {"query": query}
        if page_size:
            params["page_size"] = page_size
    ...
    async def retry_on_rate_limit(self, func, *args, max_retries: int = 3, base_delay: float = 1.0, **kwargs) -> Any:
    ...
    def create_media_object(
        ...
        if source_type == "image_url" and url:
            media["source"] = {"source_type": "image_url", "url": url}
        elif source_type == "image_base64" and data:
            media["source"] = {"source_type": "image_base64", "data": data}
        elif source_type == "video_url" and url:
            media["source"] = {"source_type": "video_url", "url": url}
    ...
    async def extract_pin_id_from_url(self, url: str) -> Optional[str]:
        patterns = [r"pinterest\.com/pin/(\d+)", r"pin\.it/(\w+)", r"/pin/(\d+)/"]
    ...
    async def get_media_urls_from_pin(self, pin_data: Dict) -> Dict[str, Any]:
        media_urls = {"images": [], "videos": [], "media_type": None}
        ...
        if "media" in pin_data:
            ...
            if "video_url" in media:
                media_urls["videos"].append({"url": media["video_url"], "type": "video"})
    ...
    async def download_media(self, url: str, save_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "path": save_path, "size": len(response.content), "url": url}
        except Exception as e:
            raise PinterestAPIError(f"Download failed: {str(e)}")

    async def download_pin_media(self, pin_id: str, save_dir: str, quality: str = "originals") -> List[Dict]:
        ...
        for idx, image_data in enumerate(media_urls["images"]):
            if image_data["size"] == quality or (quality == "best" and idx == 0):
                ...
                try:
                    ...
                    downloaded_files.append({"success": False, "error": str(e), "url": image_data["url"]})
        ...
        for idx, video_data in enumerate(media_urls["videos"]):
            ...
            try:
                ...
                downloaded_files.append({"success": False, "error": str(e), "url": video_data["url"]})

        return downloaded_files

    async def download_from_url(self, pinterest_url: str, save_dir: str, quality: str = "originals") -> List[Dict]:
    ...
    async def search_and_download(
        self, query: str, save_dir: str, max_items: int = 10, quality: str = "originals"
    ) -> List[Dict]:
        ...
        for pin in pins[:max_items]:
            ...
            if pin_id:
                try:
                    ...
                    all_downloads.append({"success": False, "pin_id": pin_id, "error": str(e)})
    ...
    async def bulk_download_from_urls(self, urls: List[str], save_dir: str, quality: str = "originals") -> List[Dict]:
        all_results = []
        for url in urls:
            try:
                ...
                all_results.append({"success": False, "url": url, "error": str(e)})

        return all_results

    async def search_images(self, query: str, page_size: Optional[int] = None, bookmark: Optional[str] = None) -> Dict:
