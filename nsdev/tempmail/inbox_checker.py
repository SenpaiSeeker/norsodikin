...
class InboxChecker:
    ...
    async def get_message_detail(self, token: str, message_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with session.get(f"{self.BASE_URL}/messages/{message_id}", headers=headers) as response:
                if response.status == 200:
    ...
    async def delete_message(self, token: str, message_id: str) -> bool:
        ...
        try:
            async with session.delete(f"{self.BASE_URL}/messages/{message_id}", headers=headers) as response:
    ...
    async def mark_as_seen(self, token: str, message_id: str) -> bool:
        ...
        try:
            async with session.patch(
                f"{self.BASE_URL}/messages/{message_id}", headers=headers, json={"seen": True}
            ) as response:
