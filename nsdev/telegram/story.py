...
class StoryDownloader:
    ...
    async def _process_and_send_story(self, story_item: types.StoryItem, target_chat_id: int, msg_id: int):
        ...
        try:
            ...
            send_method = None
            caption = (
                getattr(story_item.caption, "html", story_item.caption) if getattr(story_item, "caption", None) else ""
            )
            ...
            if isinstance(story_item.media, types.MessageMediaPhoto):
                ...
                high_level_media = Document._parse(
                    self._client, story_item.media.document, f"story_{uuid.uuid4().hex[:9]}.mp4"
                )
        ...
        try:
            ...
            if not story_ids:
                return await status_message.edit_text(
                    f"✅ Pengguna `{username}` tidak memiliki story aktif (atau tidak dapat diakses)."
                )

            total = len(story_ids)
            await status_message.edit_text(
                f"✅ Ditemukan {total} story aktif. Mengunduh dengan metode GetStoriesByID..."
            )
            ...
            if processed_count == 0:
                ...
                await status_message.edit_text(
                    f"✅ Berhasil mengunduh {processed_count}/{total} story dari `{username}`!"
                )
    ...
    async def download_single_story(
        self, username: str, story_id: int, chat_id: int, status_message: Message, message_id: int
    ):
        if self._client.me.is_bot:
            raise Exception("Bot accounts cannot download stories. This feature requires a Userbot.")
        ...
        try:
            ...
            if not story_data.stories:
                return await status_message.edit_text(
                    f"❌ Story dengan ID `{story_id}` tidak ditemukan untuk `{username}`."
                )
