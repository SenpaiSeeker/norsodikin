...
class VideoFX(FontManager):
    ...
    def _draw_lightning_bolt(self, canvas_img: Image.Image, text_bbox: Tuple) -> Image.Image:
        ...
        draw_core.line(main_path, fill=(255, 100, 0, 180), width=9, joint="round")
        draw_core.line(main_path, fill=(255, 180, 50, 220), width=5, joint="round")
        draw_core.line(main_path, fill=(255, 255, 200, 255), width=2, joint="round")
        ...
        if random.random() < 0.7:
            for _ in range(random.randint(1, 3)):
                ...
                draw_core.line(branch_path, fill=(255, 100, 0, 150), width=7, joint="round")
                draw_core.line(branch_path, fill=(255, 180, 50, 200), width=4, joint="round")
                draw_core.line(branch_path, fill=(255, 255, 200, 255), width=1, joint="round")

        aura_layer = core_layer.filter(ImageFilter.GaussianBlur(radius=20))
        AURA_COLOR = (255, 20, 0)
        solid_aura = Image.new("RGBA", canvas_size, AURA_COLOR + (0,))
        solid_aura.putalpha(aura_layer.getchannel("A"))

        final_img = Image.alpha_composite(canvas_img, solid_aura)
