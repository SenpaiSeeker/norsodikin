...
class VideoFX(FontManager):
    ...
    async def text_to_tgs(self, text: str, output_path: str, font_size: int = 100):
        clean_text = text.replace(";", "\r")
        
        layer_shake_keyframes = []
        for frame in range(0, 180, 2): 
            offset_x = 256 + random.uniform(-3, 3)
            offset_y = 256 + random.uniform(-3, 3)
            layer_shake_keyframes.append({
                "t": frame,
                "s": [offset_x, offset_y, 0],
                "h": 0
            })
        ...
        await self._run_in_executor(
            self._write_tgs_file, lottie_structure, output_path
        )
        return output_path

    def _write_tgs_file(self, data: dict, path: str):
