...
class ErrorHandler:
    ...
    def handle(self, log_channel_id: int = None, error_message_template: str = "❌ Terjadi kesalahan:\n`{error}: {message}`", silent: bool = False):
        def decorator(func):
            @wraps(func)
            async def wrapped(client, message, *args, **kwargs):
                try:
                        ...
                        try:
                            ...
                            if len(error_for_channel) > 4096:
                                ...
                                await client.send_document(
                                    log_channel_id,
                                    document=log_file,
                                    caption=f"Error log for `{func.__name__}`"
                                )
                            else:
                    ...
                    if not silent:
                        try:
                            user_error_msg = error_message_template.format(
