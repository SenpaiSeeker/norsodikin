...
class DramaBiteAPI:
    ...
    async def home(self) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{self.base_url}/home", params=params)
            return response.json()

    async def module(self, page: int = 1) -> Dict[str, Any]:
        params = {"page": str(page), "lang": self.lang}
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{self.base_url}/module", params=params)
            return response.json()
    ...
    async def detail(self, cid: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{self.base_url}/drama/{cid}", params=params)
            return response.json()

    async def episodes(self, cid: str) -> Dict[str, Any]:
        params = {"cid": cid, "lang": self.lang, "code": self.token}
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{self.base_url}/episodes/{cid}", params=params)
            return response.json()

    async def play(self, cid: str, vid: str) -> Dict[str, Any]:
        params = {"cid": cid, "vid": vid, "lang": self.lang, "code": self.token}
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{self.base_url}/play/{cid}/{vid}", params=params)
            return response.json()
