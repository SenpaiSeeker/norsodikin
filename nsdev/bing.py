class ImageGenerator:
    ...
    async def generate(self, prompt: str, num_images: int = 4, max_wait_seconds: int = 300):
        ...
        if response.status_code != 302:
            self.__log(f"{self.log.RED}Status code tidak valid: {response.status_code}. Mungkin cookie tidak valid atau layanan diblokir.")
        ...
        while True:
            ...
            if processed_urls:
                self.__log(f"{self.log.GREEN}Ditemukan {len(processed_urls)} gambar. Total waktu: {round(self.time.time() - start_time, 2)} detik.")
