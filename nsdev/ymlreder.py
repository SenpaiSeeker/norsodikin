class YamlHandler(__import__("nsdev").AnsiColors):
    ...
    def loadAndConvert(self, filePath):
        try:
            ...
            print(
                f"{self.YELLOW}File {self.LIGHT_CYAN}'{self.filePath}' {self.RED}tidak ditemukan.{self.RESET}"
            )
        except self.yaml.YAMLError as e:
            print(
                f"{self.YELLOW}Kesalahan saat memproses file YAML: {self.RED}{e}{self.RESET}"
            )
        return None
    ...
    def _convertToNamespace(self, data):
        if isinstance(data, dict):
            return self.SimpleNamespace(
                **{k: self._convertToNamespace(v) for k, v in data.items()}
            )
